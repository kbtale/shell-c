...
extern int builtin_ps(char **args, int arg_count);
...
extern int builtin_df(char **args, int arg_count);
extern int builtin_du(char **args, int arg_count);

static const char *builtins_list[] = {
    "echo", "exit", "type", "pwd", "cd", "history", "cshell", 
    "mx", "hexdump", "bindump", "weather", "help", "ls", 
    "clear", "cls", "ping", "read", "touch", "banner", "whoami", "ip", "ps", "uptime", "date", "env", "uname", "df", "du"
};
...
int execute_builtin(char **args, int arg_count, int *exit_signal) {
    ...
    if (strcmp(args[0], "ps") == 0) return builtin_ps(args, arg_count);
    ...
    if (strcmp(args[0], "df") == 0) return builtin_df(args, arg_count);
    if (strcmp(args[0], "du") == 0) return builtin_du(args, arg_count);
