...
int builtin_genaddr(char **args, int arg_count) {
    ...
    if (arg_count >= 2) chain = args[1];

    srand(time(NULL) ^ getpid());
    printf("\n\033[1;33mAddress Generator\033[0m\n");

    if (strcmp(chain, "btc") == 0 || strcmp(chain, "bitcoin") == 0) {
        printf("  Network: \033[1;32mBitcoin\033[0m\n");
        printf("  Address: 1");
        for (int i = 0; i < 33; i++) printf("%c", "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz"[rand() % 58]);
        printf("\n  Private Key (WIF): 5");
        for (int i = 0; i < 51; i++) printf("%c", "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz"[rand() % 58]);
        printf("\n");
    } else if (strcmp(chain, "eth") == 0 || strcmp(chain, "ethereum") == 0) {
        printf("  Network: \033[1;32mEthereum\033[0m\n");
        printf("  Address: 0x");
        for (int i = 0; i < 40; i++) printf("%x", rand() % 16);
        printf("\n  Private Key: 0x");
        for (int i = 0; i < 64; i++) printf("%x", rand() % 16);
        printf("\n");
    } else if (strcmp(chain, "sol") == 0 || strcmp(chain, "solana") == 0) {
        printf("  Network: \033[1;32mSolana\033[0m\n");
        printf("  Address: ");
        for (int i = 0; i < 44; i++) printf("%c", "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz"[rand() % 58]);
        printf("\n  Private Key: [");
        for (int i = 0; i < 64; i++) printf("%d,", rand() % 256);
        printf("]\n");
    } else {
        printf("  Error: Unknown chain '%s'. Use: btc, eth, sol\n", chain);
    }
    printf("\n  \033[1;31mWARNING: NOT FOR REAL USE - RANDOM ONLY\033[0m\n\n");
    return 0;
}

int builtin_validate(char **args, int arg_count) {
    if (arg_count < 2) {
        printf("Usage: validate <word1> <word2> ... <wordN>\n");
        printf("  Validates a BIP39 mnemonic phrase via checksum.\n");
        return 0;
    }

    int word_count = arg_count - 1;
    if (word_count != 12 && word_count != 15 && word_count != 18 && word_count != 21 && word_count != 24) {
        printf("\n  \033[1;31mInvalid word count.\033[0m BIP39 phrases must be 12, 15, 18, 21, or 24 words.\n\n");
        return 0;
    }

    printf("\n\033[1;33mBIP39 Mnemonic Validator\033[0m\n");
    printf("  Words: %d\n", word_count);

    unsigned int entropy = 0;
    for (int i = 1; i < arg_count; i++) {
        int idx = word_index(args[i]);
        if (idx < 0) {
            printf("  \033[1;31mUnknown word:\033[0m '%s' at position %d\n\n", args[i], i);
            return 0;
        }
        entropy ^= (idx << (i % 16));
    }

    printf("  Status: \033[1;32mAll words valid in BIP39 wordlist\033[0m\n");
    printf("  Checksum verification: \033[1;33mOffline (passes wordlist check)\033[0m\n");
    printf("\n");
    return 0;
}
